-- Phase 5: Atomic task graph CRUD RPC
-- Implements critical-path foundation for create/update/delete in one transaction.

-- 1) Pre-clean legacy duplicates so we can enforce max 1 dependency row per task.
with ranked as (
  select
    d.id,
    d.tarea_id,
    d.depende_de_id,
    row_number() over (
      partition by d.tarea_id
      order by
        case
          when t.depende_de_id is not distinct from d.depende_de_id then 0
          else 1
        end,
        d.created_at,
        d.id
  ...
  join public.tareas t on t.id = d.tarea_id
)
...
set depende_de_id = d.depende_de_id
from public.dependencias d
where d.tarea_id = t.id
  and t.depende_de_id is distinct from d.depende_de_id;

create unique index if not exists ux_dependencias_tarea_id
  on public.dependencias (tarea_id);
